...
const postNewTask = async (event) => {
    ...
    // Get task assignee
        ...
        // Get the new task button data attribute with the user id set to it
        const user_assigned_id = newTaskButton.getAttribute('data-current-user');
    ...
    // Get task assignee
      ...
      function displayProjectValue() {
        // Get all choices by name (name is an attribute on the radio inputs)
        const options = document.getElementsByName('assigneeProject');
        ...
        for (i = 0; i < options.length; i++) {
          ...
          if (options[i].checked) {
            ...
            project_id = options[i].value;
            console.log(`Detected Radio Value is ${project_id}`);
            break;
          }
        }
      }
    ...
    if (
      ...
      const response = await fetch('/api/tasks', {
        method: 'POST',
        body: JSON.stringify({ title, description, due_date, user_assigned_id, project_id }),
        headers: {
