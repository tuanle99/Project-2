...
const postNewTask = async (event) => {
  ...
  // Invoke hoisted functionn I defined right below
  displayRadioValue();
  ...
  // Invoke hoisted functionn I defined right below
  displayProjectRadioValue();

  // Define sub-functoin for checking which button is selected
  function displayProjectRadioValue() {
    // Get all choices by name (name is an attribute on the radio inputs)
    ...
    for (i = 0; i < options.length; i++) {
      // If an option is checked
      if (options[i].checked) {
        // Set its value to the project_assigned_id
  ...
  // If no content exists when posting, alert them to fill it out first
    ...
    alert(
      'Please make sure you enter the due date in the proper MM/DD/YYYY format'
    );
  } else {
    alert(
